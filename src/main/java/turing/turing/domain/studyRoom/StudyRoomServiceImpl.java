...
public class StudyRoomServiceImpl implements StudyRoomService{
    ...
    public Long createStudyRoom(Long teacherId, StudyRoomCreateReqDto studyRoomCreateReqDto) {
        ...
        studyRoom.getStudyTimes().addAll(studyTimes);  // 양방향 연관관계이므로, 양쪽 방향에 모두 값을 입력해주자 (테스트에서 문제 발생하였기 때문에 추가함)
    ...
    public void updateStudyRoom(Long studyRoomId, StudyRoomUpdateReqDto studyRoomUpdateReqDto) {
        ...
        studyRoom.getStudyTimes().addAll(studyTimes);  // 양방향 연관관계이므로, 양쪽 방향에 모두 값을 입력해주자
        studyTimeRepository.saveAll(studyTimes);
    }
    ...
    public DetailedStudyRoomResDto getDetailedStudyRoom(Long studyRoomId, Role role){
    ...
    public Integer generateCode(){
        ...
        for (int i = 0; i < maxAttempts; i++) {
            ...
            if (!connectionCodeRepository.existsByCode(generatedCode)) {
                return generatedCode;
            }
        }

        throw new RestApiException(StudyRoomErrorCode.CONNECTION_CODE_GENERATE_FAILED);
    }
